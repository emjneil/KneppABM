...
class habitatAgent (Agent):
    ...
    def step(self):
        ...
        # don't let it go over 1520 mature shrubs
        number_scrub_maturing = min(number_scrub_maturing, 1520 - self.edibles['scrub'])
        ...
        # don't let it go over 760 trees
        number_saplings_maturing = min(number_saplings_maturing, 760 - self.edibles["trees"])
        ...
        available_sapling_cell = [i for i in no_herbivores if i.edibles["saplings"] < 5000 and i.edibles["trees"] < 760]
        ...
        available_youngscrub_cell = [i for i in no_herbivores if i.edibles["youngScrub"] < 5000 and i.edibles['scrub'] < 760]
        ...
        outcompeted_by_trees = ((self.edibles["trees"]/760)*self.model.chance_grassOutcompetedByTree) 
        ...
        outcompeted_by_shrubs = ((self.edibles['scrub']/1520)*self.model.chance_grassOutcompetedByScrub)
        ...
        mature_scrub_outcompeted = np.random.binomial(n=self.edibles['scrub'], p=(self.edibles["trees"]/760)*self.model.chance_scrubOutcompetedByTree)
        ...
        outcompeted_by_trees = ((self.edibles["trees"]/760)*self.model.chance_saplingOutcompetedByTree) 
        ...
        outcompeted_by_shrubs = ((self.edibles['scrub']/1520)*self.model.chance_saplingOutcompetedByScrub)
        outcompeted_saplings_byScrub = np.random.binomial(n=self.edibles["saplings"], p=outcompeted_by_shrubs)
        self.edibles["saplings"] -= outcompeted_grass_byScrub
        self.habs_outcompeted_byScrub["saplings"] += outcompeted_grass_byScrub
        ...
        self.habs_outcompeted["youngScrub"] += outcompeted_youngscrub

        outcompeted_by_trees = ((self.edibles["trees"]/760)*self.model.chance_youngScrubOutcompetedByTree) 
        outcompeted_youngScrub_byTrees = np.random.binomial(n=self.edibles["saplings"], p=outcompeted_by_trees)
        self.edibles["saplings"] -= outcompeted_saplings_byTrees
        self.habs_outcompeted_byTrees["saplings"] += outcompeted_saplings_byTrees

        outcompeted_by_shrubs = ((self.edibles['scrub']/1520)*self.model.chance_youngScrubOutcompetedByScrub)
        outcompeted_youngScrub_byScrub = np.random.binomial(n=self.edibles["saplings"], p=outcompeted_by_shrubs)
        self.edibles["saplings"] -= outcompeted_grass_byScrub
        self.habs_outcompeted_byScrub["saplings"] += outcompeted_grass_byScrub
        ...
        if self.edibles["trees"] < 25 and self.edibles["scrub"] < 50 and self.edibles["grass"] >= 50:
            self.condition = "grassland"
        elif self.edibles["trees"] < 25 and self.edibles["scrub"] >= 50:
            self.condition = "thorny_scrubland"
        elif self.edibles["trees"] >= 25:
            self.condition = "woodland" 
        elif self.edibles["trees"] < 25 and self.edibles["scrub"] < 50 and self.edibles["bare_ground"] > 50:
        ...
        for _ in range(len(habitat_choices)):
            if self.energy < 1:
                ...
                if my_choice == "saplings":
                    ...
                    if eatenSaps >= habitat_patch.edibles["saplings"] - (round(habitat_patch.edibles["saplings"] * (habitat_patch.edibles['scrub']/1520))):
                        eatenSaps = habitat_patch.edibles["saplings"] - (round(habitat_patch.edibles["saplings"] * (habitat_patch.edibles['scrub']/1520)))
                    ...
                    # scrub facilitates saplings by preventing herbivory
                    if eatenYoungScrub >= habitat_patch.edibles["youngScrub"] - (round(habitat_patch.edibles["youngScrub"] * (habitat_patch.edibles['scrub']/1520))):
                        eatenYoungScrub = habitat_patch.edibles["youngScrub"] - (round(habitat_patch.edibles["youngScrub"] * (habitat_patch.edibles['scrub']/1520)))
...
class tamworthPigs(RandomWalker):
    ...
    def step(self):
        ...
        if len(boars_here) > 0 and living and self.pregnancy_timer == None and self.boar == False and (random.random() < self.model.pigs_reproduce/np.log10(self.model.schedule.get_breed_count(tamworthPigs)+ 1)):
...
class KneppModel(Model):
    ...
    def __init__(self,             
        ...
        for cell, condit, in zip(self.grid.coord_iter(), condition):
            ...
            if my_condition == 'grassland': # more than 50% grassland, no more than 10 mature trees/shrubs
                trees_here = random.randint(0, 24)
                saplings_here = 0
                ...
                trees_here = random.randint(0, 24)
                saplings_here = random.randint(0, round(max_start_saplings * 5000))
                scrub_here = random.randint(50, 1520)
                youngscrub_here = random.randint(0, round(max_start_youngScrub * 5000))
                ...
                trees_here = random.randint(25, 760)
                saplings_here = random.randint(0, round(max_start_saplings * 5000))
                scrub_here = random.randint(0, 152)
                youngscrub_here = random.randint(0, round(max_start_youngScrub * 5000))
                perc_grass_here = random.randint(0, 100)
                perc_bareground_here = 100 - perc_grass_here
        ...
        self.datacollector = DataCollector(
                        ...
                        # competition
                        "Outcompeted Grass": lambda m: self.count_habitats_outcompeted(m, "grass"),
                        "Outcompeted Mature Scrub": lambda m: self.count_habitats_outcompeted(m, "scrub"),
                        "Outcompeted Saplings": lambda m: self.count_habitats_outcompeted(m, "saplings"),
                        "Outcompeted Young Scrub": lambda m: self.count_habitats_outcompeted(m, "youngScrub"), 
    ...
    def count_habitats_outcompeted(self, model, habitat_type):
        ...
        for key, value in model.schedule.agents_by_breed[habitatAgent].items():
            count_item += value.habs_outcompeted[habitat_type]
        return count_item
