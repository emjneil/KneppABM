...
class KneppModel(Model):
    ...
    def add_pig(self, herbivore, count_non_boar, boar):
        x = random.randrange(self.width)
        y = random.randrange(self.height)
        energy = np.random.uniform(0, 1)
        for i in range(count):
    ...
    def remove_pig(self, herbivore, count_non_boar, is_boar):
        ...
        my_choices_boar = random.sample(list(to_remove), k = min(count, len(to_remove)))
        # remove non-boars
        for my_choice in my_choices:
            my_choice = my_choice[1]
            self.grid._remove_agent(my_choice.pos, my_choice)
