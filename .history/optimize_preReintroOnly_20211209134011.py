...
def objectiveFunction(x):
    ...
    max_start_saplings = x[18]
    max_start_youngScrub = x[19]
    ...
    if (list(results.loc[results['Time'] == 1, 'Saplings'])[0]) > (list(results.loc[results['Time'] == 0, 'Saplings'])[0])
    
    round(max_start_saplings*3000*(initial_woodland+initial_scrubland)) and (list(results.loc[results['Time'] == 1, 'Young Scrub'])[0]) > round(max_start_youngScrub*3000*(initial_woodland+initial_scrubland)):
        filtered_result = (
            # pre-reintro model
        ...
        print("young veg declining")
        filtered_result = (
        (((list(results.loc[results['Time'] == 5, 'Saplings'])[0])-round(max_start_saplings*3500*(initial_woodland+initial_scrubland)))**2) +
        (((list(results.loc[results['Time'] == 5, 'Young Scrub'])[0])-round(max_start_youngScrub*3500*(initial_woodland+initial_scrubland)))**2))
...
def run_optimizer():
    ...
    bds = np.array([
        ...
        [0,1],[0,1],[0.001,0.1],[0.0025,0.005],[0.005,0.01],
        # habitat competition
        [0,0.01],[0,0.1],[0,0.1],[0,0.5],[0,0.1],[0,0.1],[0,0.5],     
        # roe deer parameters
        [0.1,0.25],[0,0.25],[0,0.25],[0,0.25],[0.005,0.05],[0.005,0.05],
        [0,1], [0,1]
    ])

#  The best solution found:
#  [0.09452434 0.09876619 0.01505916 0.00405874 0.00991246 0.03470027
#  0.11846652 0.10243712 0.04005124 0.0008608  0.00974191 0.01457942
#  0.21472161 0.09767759 0.11597788 0.46664108 0.00361987 0.06213689]

#  Objective function:
#  2.0

#  The best solution found:                                                                           
#  [2.25290544e-01 2.82525510e-01 1.54234195e-02 2.78829975e-03
#  5.52909525e-03 1.28565688e-04 6.11796505e-02 8.78168263e-02
#  4.35159447e-01 8.68343280e-05 5.60413141e-02 3.35729491e-01
#  1.71426274e-01 1.44949238e-01 7.78720914e-02 1.94380324e-01
#  2.04022403e-02 7.03990244e-03]
...
#  5.0
    ...
    optimization =  ga(function = objectiveFunction, dimension = 20, variable_type = 'real',variable_boundaries= bds, algorithm_parameters = algorithm_param, function_timeout=6000)
    optimization.run()
...
def graph_results():
    ...
    # chance_reproduceYoungScrub = 0.025
    # chance_regrowGrass = 0.045
    # chance_saplingBecomingTree = 0.00004
    # chance_youngScrubMatures = 0.00013
    # chance_scrubOutcompetedByTree = 0.0084
    # chance_grassOutcompetedByTree = 0.0064
    # chance_grassOutcompetedByScrub = 0.0062
    # chance_saplingOutcompetedByTree = 0.0034
    # chance_saplingOutcompetedByScrub = 0.0011
    # chance_youngScrubOutcompetedByScrub = 0.0046
    # chance_youngScrubOutcompetedByTree = 0.0037
    ...
    # roeDeer_reproduce = 0.14
    # roeDeer_gain_from_grass = 0.8
    # roeDeer_gain_from_Trees = 0.12
    # roeDeer_gain_from_Scrub = 0.68
    # roeDeer_gain_from_Saplings = 0.3
    # roeDeer_gain_from_YoungScrub = 0.18
    ...
    pigs_gain_from_Saplings = 0.01
    pigs_gain_from_YoungScrub = 0.01    
    max_start_saplings = output_parameters["variable"][18]
    max_start_youngScrub = output_parameters["variable"][19]
    ...
    reality_check_0 = KneppModel(
        ...
        pigs_reproduce, pigs_gain_from_grass, pigs_gain_from_Saplings, pigs_gain_from_YoungScrub, 
        max_start_saplings, max_start_youngScrub,
        width = 50, height = 36, max_time = 2000, reintroduction = False, 
        RC1_noFood = False, RC2_noTreesScrub = False, RC3_noTrees = False, RC4_noScrub = False)
    ...
    species_list_ages = np.tile(["Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Grass", "Trees", "Mature Scrub", "Saplings", "Young Scrub", "Bare Areas"], 2001) 
    ...
    species_list_ages1 = np.tile(["Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Grassland", "Woodland", "Thorny Scrub", "Bare ground"], 2001) 
