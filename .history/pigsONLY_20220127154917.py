...
def objectiveFunction(x):
    ...
    ponies_gain_from_grass = x[0]
    ponies_gain_from_Trees = x[1]
    ponies_gain_from_Scrub =  x[2]
    ponies_gain_from_Saplings = x[3]
    ponies_gain_from_YoungScrub = x[4]
    pigs_reproduce = x[5]
    pigs_gain_from_grass =x[6]
    pigs_gain_from_Saplings =x[7]
    pigs_gain_from_YoungScrub = x[8]
...
def run_optimizer():
    ...
    bds = np.array([
        ...
        [0.4,0.5],[0.3,0.5],[0.3,0.5],[0.05,0.1],[0.05,0.1],
        # # cattle parameters
        # [0,0.3],[0,0.15],[0,0.15],[0,0.15],[0,0.01],[0,0.01],
        # # pig parameters
        [0.01,0.2],[0,0.1],[0,0.1],[0,0.1]
    ])


    algorithm_param = {'max_num_iteration': 102,\
                    'population_size':50,\
    ...
    optimization =  ga(function = objectiveFunction, dimension = 9, variable_type = 'real',variable_boundaries= bds, algorithm_parameters = algorithm_param, function_timeout=6000)
    optimization.run()
...
def graph_results():
    ...
    redDeer_gain_from_Saplings = 0.01
    redDeer_gain_from_YoungScrub = 0.01
    ponies_gain_from_grass = output_parameters["variable"][0]
    ponies_gain_from_Trees =output_parameters["variable"][1]
    ponies_gain_from_Scrub =output_parameters["variable"][2]
    ponies_gain_from_Saplings =output_parameters["variable"][3]
    ponies_gain_from_YoungScrub =output_parameters["variable"][4]
    cows_reproduce = 0
    cows_gain_from_grass = 0.01
    cows_gain_from_Trees = 0.01
    cows_gain_from_Scrub = 0.01
    ...
    pigs_reproduce = output_parameters["variable"][5]
    pigs_gain_from_grass = output_parameters["variable"][6]
    pigs_gain_from_Saplings =output_parameters["variable"][7]
    pigs_gain_from_YoungScrub =output_parameters["variable"][8]
...
# y values - number of trees, scrub, etc. 
    ...
    "Grass Outcompeted by Trees", "Grass Outcompeted by Scrub", "Grass eaten by roe deer", "Grass eaten by Exmoor pony", "Grass eaten by Fallow deer", "Grass eaten by longhorn cattle", "Grass eaten by red deer", "Grass eaten by pigs", 
     ...
     "Scrub eaten by red deer", "Trees eaten by roe deer", "Trees eaten by Exmoor pony", "Trees eaten by Fallow deer", "Trees eaten by longhorn cattle",  "Trees eaten by red deer"], axis=1).values.flatten()           
    ...
    plt.tight_layout()
    plt.savefig('OptimOutputs_PostReintro_numbers_pigsOnly.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('OptimOutputs_PostReintro_conditions_pigsOnly.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('RealityCheck_run1k_PostReintro_pigsOnly.png')
    plt.show()
...
# Who's eating what
    ...
    habitat_type = np.tile(["Sapling", "Sapling", "Sapling", "Sapling","Sapling", "Sapling", "Sapling", "Sapling", "Sapling", "Young Scrub", "Young Scrub", "Young Scrub", "Young Scrub", "Young Scrub", "Young Scrub", "Young Scrub", "Young Scrub", "Young Scrub", "Grass", "Grass", "Grass", "Grass", "Grass", "Grass", "Grass", "Grass", "Scrub", "Scrub","Scrub","Scrub","Scrub","Scrub", "Tree", "Tree", "Tree", "Tree", "Tree"], 185) 
    death_by = np.tile(["Saplings grown up", "Saplings Outcompeted by Trees", "Saplings Outcompeted by Scrub", "Saplings eaten by roe deer", "Saplings eaten by Exmoor pony", "Saplings eaten by Fallow deer", "Saplings eaten by longhorn cattle", "Saplings eaten by red deer",  "Saplings eaten by pigs", "Young scrub grown up", "Young Scrub Outcompeted by Trees", "Young Scrub Outcompeted by Scrub", "Young Scrub eaten by roe deer", "Young Scrub eaten by Exmoor pony", "Young Scrub eaten by Fallow deer", "Young Scrub eaten by longhorn cattle", "Young Scrub eaten by red deer", "Young Scrub eaten by pigs", "Grass Outcompeted by Trees", "Grass Outcompeted by Scrub", "Grass eaten by roe deer",  "Grass eaten by Exmoor pony", "Grass eaten by Fallow deer", "Grass eaten by longhorn cattle", "Grass eaten by red deer", "Grass eaten by pigs",
     "Scrub Outcompeted by Trees", "Scrub eaten by roe deer", "Scrub eaten by Exmoor pony", "Scrub eaten by Fallow deer", "Scrub eaten by longhorn cattle", "Scrub eaten by red deer", 
     "Trees eaten by roe deer", "Trees eaten by Exmoor pony", "Trees eaten by Fallow deer", "Trees eaten by longhorn cattle",  "Trees eaten by red deer"], 185)
    ...
    stacked_time = np.repeat(final_results['Time'], 37)
    ...
    g = sns.FacetGrid(final_df_stacked, col = 'Ecosystem Element', hue = 'Death By', palette = "colorblind")
    g = (g.map(sns.barplot, 'Time', 'Amount Died', ci = None).add_legend())
    g.set(xticks=np.arange(0,185,25))  
    plt.savefig('Eaten_Food_PostReintro_pigsOnly.png')
    plt.show()
