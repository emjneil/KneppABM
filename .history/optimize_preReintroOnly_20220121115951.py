...
def graph_results():
    ...
    stacked_habitats = final_results.drop(['Time', "Bare ground","Grassland", "Woodland", "Thorny Scrub", "Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Grass", "Trees", "Mature Scrub", "Saplings", "Young Scrub", "Bare Areas"], axis=1).values.flatten()        
    ...
    {'Abundance': stacked_habitats, 'Ecosystem Element': stacked_list, 'Time': stacked_indices})
    print(final_df_stacked)

    g = sns.FacetGrid(final_df_stacked, col = 'Ecosystem Element')
    g = (g.map(sns.barplot, 'Time', 'Abundance', ci = None).add_legend())
