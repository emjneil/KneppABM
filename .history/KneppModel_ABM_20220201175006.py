...
class tamworthPigs(RandomWalker):
    ...
    def giveBirth(self):
        ...
        for _ in range(random.randint(1,10)):
            piglet = tamworthPigs(self.model.next_id(), self.pos, self.model, self.moore, self.energy, self.condition == "piglet")
...
class KneppModel(Model):
    ...
    def count_condition(self, model, condition):
        # want to count grass, wood, scrub, bare ground in each patch
        count = 0
        for key, value in model.schedule.agents_by_breed[tamworthPigs(unique_id, pos, model, moore, energy, condition)].items():
