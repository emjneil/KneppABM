...
def graph_results():
    ...
    stacked_habitats = final_results.drop(['Time', "Bare ground","Grassland", "Woodland", "Thorny Scrub", "Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Trees", "Mature Scrub", "Saplings", "Young Scrub", "Bare Areas"], axis=1).values.flatten()        
    stacked_eco_element = np.tile(["Grass", 51])

    stacked_list = np.tile(["Grass", "Grown saplings", "Grown young scrub", "Outcompeted Grass", "Outcompeted Mature Scrub", "Outcompeted Saplings", "Outcompeted Young Scrub", "Outcompeted Grass-Scrub", "Outcompeted Young Scrub-Scrub", "Outcompeted Saplings-Scrub", "Eaten Grass", "Eaten Trees", "Eaten Mature Scrub", "Eaten Saplings", "Eaten Young Scrub"], 51)
    stacked_indices = np.repeat(final_results['Time'], 14)
    final_df_stacked = pd.DataFrame(
    {'Abundance': stacked_habitats, 'Ecosystem Element': stacked_eco_element, 'Competition': stacked_list, 'Time': stacked_indices})
