...
def graph_runs():
    ...
    y_values_finaldf = numbers_results.drop(['run_number', 'accepted?', 'Time'], axis=1).values.flatten()
    y_values_counterfactual = counterfactual_results.drop(['run_number', 'accepted?', 'Time'], axis=1).values.flatten()
    y_values_numbers = np.concatenate((y_values_finaldf, y_values_counterfactual), axis=0)
