...
class tamworthPigs(RandomWalker):
    def __init__(self, unique_id, pos, model, moore, energy):
        ...
        # self.boar = random.choice([True, False]) update this
    ...
    def step(self):
        ...
        # pigs reproduce Jan - July (1 - 7, < 8); and self.boar == False
        if living and self.pregnancy_timer == None and (random.random() < self.model.pigs_reproduce/np.log10(self.model.schedule.get_breed_count(tamworthPigs)+ 1)) and (1 <= self.model.get_month() < 8):
            self.pregnancy_timer = 3
            # divide my energy
            self.energy = np.random.uniform(0, self.energy)
...
class KneppModel(Model):
    ...
    def add_pig(self, herbivore, count_non_boar, count_boar):
        ...
        for i in range(count_non_boar):
            to_add = herbivore(self.next_id(), (x, y), self, True, energy, boar == False)
            self.grid.place_agent(to_add, (x, y))
            self.schedule.add(to_add)
        for i in range(count_boar):
            to_add = herbivore(self.next_id(), (x, y), self, True, energy, boar == True)
            self.grid.place_agent(to_add, (x, y))
            self.schedule.add(to_add)


    def remove_pig(self, herbivore, count_non_boar, count_boar):
        to_remove = self.schedule.agents_by_breed[herbivore.boar == False].items()
        to_remove_boars = self.schedule.agents_by_breed[herbivore.boar == True].items()
        my_choices = random.sample(list(to_remove), k = min(count_non_boar, len(to_remove)))
        ...
        for my_choice_boar in my_choices_boar:
            my_choice_boar = my_choice_boar[1]
        ...
        if self.reintroduction == True: 
            ...
            if self.schedule.time == 61: 
                ...
                if pigsValue >= 17:
                    ...
                    self.remove_pigs(tamworthPigs, number_to_subtract, 0)
                else:
                    number_to_add = 17 - pigsValue
                    self.add_pigs(tamworthPigs, number_to_add, 0)
            # 2011
            ...
            if self.schedule.time == 119:
                self.add_pigs(tamworthPigs, 0, 1)
            ...
            if self.schedule.time == 122:
                # April 2015: one pig culled - assume this is the boar, 5 born
                self.remove_pigs(tamworthPigs, 0, 1)
            # May 2015
            if self.schedule.time == 123:
                # May 2015: 8 pigs culled
                self.remove_pigs(tamworthPigs, 8, 0)
            ...
            if self.schedule.time == 142:
                ...
                self.remove_pigs(tamworthPigs, 4, 0)
            ...
            if self.schedule.time == 144:
                ...
                self.remove_pigs(tamworthPigs, 2, 1)
            ...
            if self.schedule.time == 155:
                self.remove_pigs(tamworthPigs, 9, 0)
                self.add_pigs(tamworthPigs, 0, 1)
            ...
            if self.schedule.time == 161:
                self.remove_herbivores(redDeer, 1)
                self.remove_pigs(tamworthPigs, 1, 0)
            ...
            if self.schedule.time == 169:
                self.remove_pigs(tamworthPigs, 1)
            ...
            if self.schedule.time == 173:
                self.remove_herbivores(tamworthPigs, 27)
