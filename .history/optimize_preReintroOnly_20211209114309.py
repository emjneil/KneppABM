...
def objectiveFunction(x):
    ...
    filtered_result = (
        # young scrub and saplings shouldn't decline at the outset
        (((list(results.loc[results['Time'] == 1, 'Saplings'])[0])-23)**2) +
        (((list(results.loc[results['Time'] == 1, 'Young Scrub'])[0])-23)**2) +
        # pre-reintro model
    ...
    if filtered_result < 100 and ((list(results.loc[results['Time'] == 1, 'Saplings'])[0])-23) > max_start_saplings and ((list(results.loc[results['Time'] == 1, 'Young Scrub'])[0])-23) > max_start_youngScrub:
    ...
    if ((list(results.loc[results['Time'] == 1, 'Saplings'])[0])-23) < max_start_saplings:
        print("saplings too low")
        return 1000
    if ((list(results.loc[results['Time'] == 1, 'Young Scrub'])[0])-23) < max_start_youngScrub:
        print("young scrub too low")
        return 1000
...
def run_optimizer():
    ...
    bds = np.array([
        ...
        [0.01,0.5],[0.01,0.5],[0.001,0.1],[0.0025,0.005],[0.005,0.01],
        # habitat competition
        [0,0.01],[0,0.1],[0,0.1],[0,0.5],[0,0.25],[0,0.25],[0,0.5],     
        # roe deer parameters
