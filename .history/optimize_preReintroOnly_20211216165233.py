...
def objectiveFunction(x):
    ...
    if (list(results.loc[results['Time'] == 5, 'Saplings'])[0]) >= (list(results.loc[results['Time'] == 0, 'Saplings'])[0]) and (list(results.loc[results['Time'] == 5, 'Young Scrub'])[0]) >= (list(results.loc[results['Time'] == 0, 'Young Scrub'])[0]):
        filtered_result = (
            # pre-reintro model
...
def run_optimizer():
    ...
    bds = np.array([
        ...
        [0.01,0.1],[0.05,0.15],[0.025,0.05],[0.0025,0.005],[0.005,0.008],
        # habitat competition
        [0.0005,0.001],[0.05,0.075],[0.001,0.1],[0,0.15],[0,0.05],[0,0.05],[0,0.15],     
        # roe deer parameters
        [0.15,0.2],[0.1,0.5],[0.1,0.5],[0.1,0.5],[0.01,0.025],[0.01,0.025],
...
#  The best solution found:                                                                           
#  [0.08283883 0.13698037 0.04245443 0.00378435 0.00793384 
# 0.00083853 0.06751125 0.06594869 0.12111045 0.09903051 0.0936203  0.13381695
#  0.20038194 0.16734465 0.16877135 0.22793852 0.01199058 0.01822774
#  0.03413711 0.07840114]
...
def graph_results():
    ...
    reality_check_0 = KneppModel(
        ...
        pigs_reproduce, pigs_gain_from_grass, pigs_gain_from_Saplings, pigs_gain_from_YoungScrub, 
        max_start_saplings, max_start_youngScrub,
        width = 50, height = 36, max_time = 1500, reintroduction = False, 
        RC1_noFood = False, RC2_noTreesScrub = False, RC3_noTrees = False, RC4_noScrub = False)
    ...
    species_list_ages = np.tile(["Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Grass", "Trees", "Mature Scrub", "Saplings", "Young Scrub", "Bare Areas"], 1501) 
    ...
    species_list_ages1 = np.tile(["Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Grassland", "Woodland", "Thorny Scrub", "Bare ground"], 1501) 
