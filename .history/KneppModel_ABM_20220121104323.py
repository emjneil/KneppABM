...
class habitatAgent (Agent):
    def __init__(self, unique_id, pos, model, condition, trees_here, saplings_here, scrub_here, youngscrub_here, perc_grass_here, perc_bareground_here):
        ...
        self.habs_outcompeted_by_trees = defaultdict(int)
        self.habs_outcompeted_by_scrub = defaultdict(int)
    ...
    def step(self):
        ...
        self.habs_outcompeted_by_trees.clear()
        self.habs_outcompeted_by_scrub.clear()
        ...
        combined_trees_shrubs = ((self.edibles["trees"]/760)*self.model.chance_grassOutcompetedByTree) + ((self.edibles['scrub']/1520)*self.model.chance_grassOutcompetedByScrub)
        ...
        combined_saplings = ((self.edibles["trees"]/760)*self.model.chance_saplingOutcompetedByTree) + ((self.edibles['scrub']/1520)*self.model.chance_saplingOutcompetedByScrub)
        ...
        combined_scrub = ((self.edibles["trees"]/760)*self.model.chance_youngScrubOutcompetedByTree) + ((self.edibles['scrub']/1520)*self.model.chance_youngScrubOutcompetedByScrub)
...
class KneppModel(Model):
    ...
    def __init__(self,             
        ...
        self.datacollector = DataCollector(
                        ...
                        "Eaten Grass": lambda m: self.count_eaten(m, roeDeer_agent, self.count_eaten, "grass"),
                        "Eaten Trees": lambda m: self.count_eaten(m, roeDeer_agent, self.count_eaten, "trees"),
                        "Eaten Mature Scrub": lambda m: self.count_eaten(m, roeDeer_agent, self.count_eaten, "scrub"),
                        "Eaten Saplings": lambda m: self.count_eaten(m, roeDeer_agent, self.count_eaten, "saplings"),
                        "Eaten Young Scrub": lambda m: self.count_eaten(m, roeDeer_agent,self.count_eaten,  "youngScrub"), 
                        # competition
                        # death by growing up (saplings, young scrub)
                        # "Saplings that grew up": lambda m: self.count_eaten(m, "saplings"),
                        # "Young scrub that grew up": lambda m: self.count_eaten(m, "youngScrub"),    
                        }
                        )
