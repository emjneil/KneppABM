...
def graph_runs():
    ...
    # accepted shape
    accepted_shape_numbers = np.repeat(numbers_results['accepted?'], 12)
    ...
    plt.tight_layout()
    plt.savefig('outputs_numbers.png')
    plt.show()
    ...
    # calculate median 
    m = filtered_final_numbers_counter.groupby(['Time', 'runType', 'Ecosystem Element'])[['Abundance %']].apply(np.median)
    m.name = 'Median'
    filtered_final_numbers_counter = filtered_final_numbers_counter.join(m, on=['Time', 'runType', 'Ecosystem Element'])
    # calculate quantiles
    perc1 = filtered_final_numbers_counter.groupby(['Time', 'runType', 'Ecosystem Element'])['Abundance %'].quantile(.95)
    perc1.name = 'ninetyfivePerc'
    filtered_final_numbers_counter = filtered_final_numbers_counter.join(perc1, on=['Time', 'runType', 'Ecosystem Element'])
    perc2 = filtered_final_numbers_counter.groupby(['Time', 'runType', 'Ecosystem Element'])['Abundance %'].quantile(.05)
    perc2.name = "fivePerc"
    filtered_final_numbers_counter = filtered_final_numbers_counter.join(perc2, on=['Time','runType', 'Ecosystem Element'])
    # reset the index
    filtered_final_numbers_counter = filtered_final_numbers_counter.reset_index(drop=True)
    colors = ["#6788ee", "#e26952"]
    # now graph it
    g = sns.FacetGrid(filtered_final_numbers_counter, col="Ecosystem Element", hue = "runType", palette = colors, col_wrap=4, sharey = False)
    g.map(sns.lineplot, 'Time', 'Median')
    g.map(sns.lineplot, 'Time', 'fivePerc')
    g.map(sns.lineplot, 'Time', 'ninetyfivePerc')
    for ax in g.axes.flat:
    ...
    axes[11].set_title("Bare ground")
    # stop the plots from overlapping
    g.fig.suptitle("Number of Habitat Types, Reintroduction vs. No Reintroduction")
    plt.legend(labels=['Rejected Runs', 'Counter'],bbox_to_anchor=(2.2, 0), loc='lower right', fontsize=12)
    plt.tight_layout()
    plt.savefig('outputs_numbers.png')
    plt.show()
