...
def run_optimizer():
    ...
    bds = np.array([
        ...
        [0.015,0.05],[0.015,0.05],[0.075,0.1],[0.002,0.004],[0.005,0.05],
        [0.005,0.025], # mature scrub competition
        [0.025,0.06],[0.025,0.06], # grass
        [0.015,0.05],[0.015,0.05], # saplings
        [0.015,0.05],[0.015,0.05], # young scrub  
        # roe deer parameters
        [0.15,0.21],[0.3,0.75],[0.3,1],[0.3,1],[0.1,0.25],[0.1,0.25]
    ])
...
def graph_results():
    ...
    g = (g.map(sns.barplot, 'Time', 'Amount Died', ci = None,  col_wrap=3).add_legend())
