...
def run_optimizer():
    ...
    bds = np.array([
        ...
        [0,0.5],[0,0.15],[0.1,0.2],[0.1,0.2],[0.001,0.05],[0.001,0.05],
        # red deer parameters
        [0,0.5],[0,0.15],[0.05,0.2],[0.05,0.2],[0,0.05],[0,0.05],
        # exmoor pony parameters
        [0.01,0.15],[0,0.1],[0.01,0.15],[0,0.01],[0,0.01],
        # cattle parameters
        [0,0.5],[0,0.1],[0,0.15],[0,0.15],[0,0.01],[0,0.01],
        # # pig parameters
        [0,1],[0,0.05],[0,0.1],[0,0.1],[0,0.1]
    ])
       roeDeer_gain_from_grass = 0.11475806
    roeDeer_gain_from_Trees = 0.11513804
    roeDeer_gain_from_Scrub = 0.10102175
...
def graph_results():
    ...
    model = KneppModel(
        ...
        pigs_reproduce, pigs_gain_from_grass, pigs_gain_from_Saplings, pigs_gain_from_YoungScrub, pigs_gain_from_soil, 
        max_start_saplings, max_start_youngScrub,
        width = 25, height = 18, max_time = 50, reintroduction = True, 
        RC1_noFood = False, RC2_noTreesScrub = False, RC3_noTrees = False, RC4_noScrub = False)
    model.run_model()
