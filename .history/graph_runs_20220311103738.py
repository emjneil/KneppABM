...
def graph_runs():
    ...
    # NUMBERS GRAPHS #
    numbers_results = final_results[["Time", "Roe deer", "Exmoor pony", "Fallow deer", "Longhorn cattle", "Red deer", "Tamworth pigs", "Grass", "Mature Trees", "Mature Scrub", "Saplings", "Young Scrub", "Bare Areas", "accepted?", "run_number"]]
    # accepted shape
    ...
    filtered_rejectedAccepted_numbers = final_df_numbers.loc[(final_df['runType'] == "Accepted") | (final_df['runType'] == "Rejected") ]
    g = sns.FacetGrid(filtered_rejectedAccepted, col="Ecosystem Element", hue = "runType", palette = colors, col_wrap=4, sharey = False)
    g.map(sns.lineplot, 'Time', 'Median')
    g.map(sns.lineplot, 'Time', 'fivePerc')
    g.map(sns.lineplot, 'Time', 'ninetyfivePerc')
    ...
    # stop the plots from overlapping
    g.fig.suptitle("Numbers in Accepted and Rejected Runs")
    plt.tight_layout()
