...
def objectiveFunction(x):
    ...
    if (list(results.loc[results['Time'] == 1, 'Saplings'])[0]) > round(max_start_saplings*3000*(initial_woodland+initial_scrubland)) and (list(results.loc[results['Time'] == 1, 'Young Scrub'])[0]) > round(max_start_youngScrub*3000*(initial_woodland+initial_scrubland)):
        filtered_result = (
            # pre-reintro model
