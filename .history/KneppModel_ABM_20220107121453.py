...
def eat_saplings(habitat_patch, eatenSaps):
    ...
    habitat_patch.sapling_deaths["eaten by roe"] += eatenSaps
...
def eat_youngscrub(habitat_patch, eatenYoungScrub):
    ...
    habitat_patch.scrub_deaths["eaten by roe"] += eatenYoungScrub
...
class habitatAgent (Agent):
    def __init__(self, unique_id, pos, model, condition, trees_here, saplings_here, scrub_here, youngscrub_here, perc_grass_here, perc_bareground_here, scrub_deaths, sapling_deaths):
        ...
        # check what they die from
        self.scrub_deaths["tree and scrub competition"] = 0
        self.scrub_deaths["eaten by roe"] = 0

        self.sapling_deaths["tree and scrub competition"] = 0
        self.sapling_deaths["eaten by roe"] = 0
    ...
    def step(self):
        ...
        self.sapling_deaths["tree and scrub competition"] += outcompeted_saplings
        ...
        self.scrub_deaths["tree and scrub competition"]
...
class KneppModel(Model):
    ...
    def __init__(self,             
        ...
        for cell, condit, in zip(self.grid.coord_iter(), condition):
            ...
            scrub_deaths = 0
            sapling_deaths = 0
            patch = habitatAgent(self.next_id(), (x, y), self, my_condition, trees_here, saplings_here, scrub_here, youngscrub_here, perc_grass_here, perc_bareground_here, scrub_deaths, sapling_deaths)
        ...
        self.datacollector = DataCollector(
                        ...
                        # what's killing young scrub and saplings
                        "Sapling deaths from competition": lambda m: self.count_deaths(m, sapling_deaths, "tree and scrub competition"),
                        "Sapling deaths from roe deer": lambda m: self.count_deaths(m, sapling_deaths, "eaten by roe"),
                        "Scrub deaths from competition": lambda m: self.count_deaths(m, scrub_deaths, "tree and scrub competition"),
                        "Scrub deaths from roe deer": lambda m: self.count_deaths(m, scrub_deaths, "eaten by roe")
                        }
                        )
    ...
    def count_deaths(self, model, type, habitat_thing):
        ...
        for key, value in model.schedule.agents_by_breed[habitatAgent].items():
            count_item += value.type[habitat_thing]
