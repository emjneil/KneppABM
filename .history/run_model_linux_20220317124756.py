...
def run_all_models():
    ...
    for index, row in final_results.iterrows():
        row.loc[df['column_name'] == some_value]
        print(row)
        if (row["Time"] == 50) & (row["Roe deer"] <= 40) & (row["Roe deer"] >= 6) & (row["Grassland"] <= 90) & (row["Grassland"] >= 49) & (row["Woodland"] <= 27) & (row["Woodland"] >= 7) & (row["Thorny Scrub"] <= 21) & (row["Thorny Scrub"] >= 1):
