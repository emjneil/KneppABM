...
def objectiveFunction(x):
    ...
    cows_reproduce =x[5] 
    cows_gain_from_grass = x[6] 
    cows_gain_from_Trees = x[7] 
    cows_gain_from_Scrub = x[8] 
    cows_gain_from_Saplings = x[9] 
    cows_gain_from_YoungScrub = x[10] 
    ...
    if filtered_result < 10000:
        ...
        with pd.option_context('display.max_columns',None):
            ...
            print(just_nodes1[["Exmoor pony", "Longhorn cattle"]])
            print(just_nodes2[["Exmoor pony", "Longhorn cattle"]])
            print(just_nodes3[["Exmoor pony", "Longhorn cattle"]])
...
def run_optimizer():
    ...
    bds = np.array([
        ...
        # [0.15,0.4],[0.15,0.5],[0.1,0.25],[0.1,0.25],[0.005,0.1],[0.005,0.1],
        # # red deer parameters
        # [0.05,0.25],[0.001,0.15],[0,0.1],[0,0.1],[0,0.01],[0,0.015],
        # # exmoor pony parameters
        [0.025,0.1],[0.075,0.15],[0.075,0.15],[0.001,0.0025],[0.001,0.0025],
        # # cattle parameters
        [0.15,0.25],[0.01,0.15],[0.05,0.15],[0.05,0.15],[0.00075,0.0025],[0.00075,0.0025],
        # # pig parameters
        # [0.01,0.5],[0.0001,0.1],[0,0.005],[0,0.005]
    ])
...
#  The best solution found: - cattle and ponies
#  [0.0273575  0.12246246 0.13849434 0.00212338 0.00078161 0.23772537
#  0.16531739 0.11693853 0.01605945 0.00731418 0.00952148]
#  Objective function:
#  6034.0
...
#  7.0
    ...
    optimization =  ga(function = objectiveFunction, dimension = 11, variable_type = 'real',variable_boundaries= bds, algorithm_parameters = algorithm_param, function_timeout=6000)
    optimization.run()
...
def graph_results():
    ...
    cows_reproduce = output_parameters["variable"][5]
    cows_gain_from_grass = output_parameters["variable"][6]
    cows_gain_from_Trees = output_parameters["variable"][7]
    cows_gain_from_Scrub =output_parameters["variable"][8]
    cows_gain_from_Saplings = output_parameters["variable"][9] 
    cows_gain_from_YoungScrub = output_parameters["variable"][10] 
