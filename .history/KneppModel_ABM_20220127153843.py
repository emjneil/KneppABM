...
class tamworthPigs(RandomWalker):
    ...
    def step(self):
        ...
        if len(boars_here) > 0 and self.condition == "sow" and living and self.pregnancy_timer == None and self.boar == False and (random.random() < self.model.pigs_reproduce/np.log10(self.model.schedule.get_breed_count(tamworthPigs)+ 1)):
...
class KneppModel(Model):
    ...
    def remove_pig(self, herbivore, count_piglets, count_sow, count_boar):
        ...
        my_choice_piglet = random.sample(list(to_remove_notBoars), k = min(count_piglets, len(to_remove_piglets)))
        my_choice_sow = random.sample(list(to_remove_notBoars), k = min(count_sow, len(to_remove_sow)))
        ...
        for my_choice in my_choice_piglet:
            ...
            self.grid._remove_agent(my_choice.pos, my_choice)
            self.schedule.remove(my_choice)
        for my_choice_boar in my_choice_sow:
            my_choice_sow = my_choice_sow
    ...
    def step(self):
        ...
        if self.reintroduction == True: 
            ...
            if self.schedule.time == 49:
                ...
                self.add_pig(tamworthPigs, 0, 20, 0)
            # March 2010
            if self.schedule.time == 61: 
                ...
                if pigsValue >= 17:
                    ...
                    self.remove_pig(tamworthPigs, 0, number_to_subtract, 0)
                else:
            ...
            if self.schedule.time == 121:
                ...
                if pigsValue >= 18:
                    number_to_subtract_sows = -5 + pigsValue
                    number_to_subtract_piglets = -13 + pigsValue
                    self.remove_pig(tamworthPigs, number_to_subtract_piglets, number_to_subtract_sows, 0)
                else:
                    number_to_add_sows = 5 - pigsValue
                    number_to_add_piglets = 13 - pigsValue
                    self.add_pig(tamworthPigs, number_to_add_piglets, number_to_add_sows, 0)
                # Red deer: 13
