...
def run_all_models():
    ...
    accepted_runs = []
    for index, row in my_time.iterrows():
        if (row["Tamworth pigs"] <= 29) & (row["Tamworth pigs"] >= 9) &
    (row["Exmoor pony"] <= 17) & (row["Exmoor pony"] >= 14) &
    (row["Longhorn cattle"] <= 106) & (row["Longhorn cattle"] >= 56) &
    (row["Roe deer"] <= 80) & (row["Roe deer"] >= 20) & 
    (row["Grassland"] <= 69) & (row["Grassland"] >= 49) & 
    (row["Thorny Scrub"] <= 35) & (row["Thorny Scrub"] >= 21) &
    (row["Woodland"] <= 29) & (row["Woodland"] >= 9):
            accepted_runs.append(row["run_number"])
    final_results['passed_filters'] = np.where(final_results['run_number'].isin(accepted_runs),final_results['passed_filters']+1,final_results['passed_filters']) 
