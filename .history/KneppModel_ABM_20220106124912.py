...
class tamworthPigs(RandomWalker):
    ...
    def step(self):
        ...
        if living and self.pregnancy_timer and (random.random() < self.model.pigs_reproduce/np.log10(self.model.schedule.get_breed_count(tamworthPigs)+ 1)) and (1 <= self.model.get_month() < 8):
            # divide my energy
            self.energy = np.random.uniform(0, self.energy)
            giveBirth()
