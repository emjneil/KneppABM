...
def run_all_models():
    ...
    final_results['run_number'] = np.where(final_results['run_number'].isin(accepted_runs),final_results['run_number']+1,df['joinkey']) 

    final_results.loc[~final_results["run_number"].isin(accepted_runs), 'passed_filters']

    # final_results = final_results.loc[final_results.run_number.isin(accepted_runs), "passed filters"] == 1

    # if final_results['run_number'].isin(accepted_runs):
