        ...
        for _ in range(len(habitat_choices)):
            if self.energy < 1:
                ...
                if my_choice == "saplings":
                    ...
                    if eatenGrass >= self.model.max_grass_roeDeer: 
                        eatenGrass = self.model.max_grass_roeDeer
...
class longhornCattle(RandomWalker):
    ...
    def step(self):
        ...
        if living and (random.random() < self.model.cows_reproduce) and (4 <= self.model.get_month() < 7):
...
class fallowDeer(RandomWalker):
    ...
    def step(self):
        ...
        if living and (random.random() < self.model.fallowDeer_reproduce) and (5 <= self.model.get_month() < 7):
...
class redDeer(RandomWalker):
    ...
    def step(self):
        ...
        # I reproduce in May & June (assuming model starts in Jan at beginning of year, May & June = time steps 5&6 out of every 12 months)
        if living and (random.random() < self.model.redDeer_reproduce) and (5 <= self.model.get_month() < 7):
            # Create a new roe deer and divide energy:
            self.energy = np.random.uniform(0, self.energy)
...
class tamworthPigs(RandomWalker):
    ...
    def step(self):
        ...
        if living and (random.random() < self.model.pigs_reproduce) and (1 <= self.model.get_month() < 8):
            # divide my energy
            self.energy = np.random.uniform(0, self.energy)
...
class habitatAgent (Agent):
    ...
    def step(self):
        ...
        # chance of tree or mature scrub dying (100 yrs)
        #   Tree = average of the following: 
        #   Scrub = average of the following: hawthorne = 250yrs; blackthorne 100; blackberry = 40; gorse = 25; elder 60 years; juniper 200;
        old_trees = np.random.binomial(n=self.edibles["trees"], p=0.008)
...
class KneppModel(Model):
    ...
    max_grass_roeDeer = 100 # assume roe deer don't kill more than 25% of grass per month
    # do this for other herbivores too


    # check number of trees, saplings, etc. in graph
    ...
    def __init__(self,             
        ...
        for cell, condit, in zip(self.grid.coord_iter(), condition):
            ...
            i, x, y = cell
            my_condition = condit
            if my_condition == 'grassland' and self.RC1_noFood == False and self.RC2_noTreesScrub == False and self.RC3_noTrees == False and self.RC4_noScrub == False: # more than 50% grassland, no more than 10 mature trees/shrubs
                ...
                perc_grass_here = random.randint(50, 100)
                perc_bareground_here = 100 - perc_grass_here
            elif my_condition == 'grassland' and self.RC1_noFood == True or self.RC2_noTreesScrub == True or self.RC3_noTrees == True and self.RC4_noScrub == True: # more than 50% grassland, no more than 10 mature trees/shrubs
                trees_here = 0
                saplings_here = 0
                scrub_here = 0
                youngscrub_here = 0
                ...
                perc_grass_here = random.randint(0, 100)
                perc_bareground_here = 100 - perc_grass_here
            elif my_condition == "bare_ground" and self.RC1_noFood == False and self.RC2_noTreesScrub == False and self.RC3_noTrees == False and self.RC4_noScrub == False: # more than 50% bare ground
                # trees_here = random.randint(0, 49)
                # saplings_here = random.randint(0, 3000)
                # scrub_here = random.randint(0, 49)
                # youngscrub_here = random.randint(0, 3000)
                ...
                perc_bareground_here = random.randint(51, 100)
                perc_grass_here = 100 - perc_bareground_here
            elif my_condition == "bare_ground" and self.RC1_noFood == True or self.RC2_noTreesScrub == True or self.RC3_noTrees == True or self.RC4_noScrub == True: # more than 50% bare ground
                trees_here = 0
                saplings_here = 0
                scrub_here = 0
                youngscrub_here = 0
        ...
        self.datacollector = DataCollector(
                        ...
                        # # number in woodland
                        # "Trees_Woodland": lambda m: self.count_food(m, "trees", "woodland"),
                        # "Saplings_Woodland": lambda m: self.count_food(m, "saplings", "woodland"),
                        # "Scrub_Woodland": lambda m: self.count_food(m, "scrub", "woodland"),
                        # "youngScrub_Woodland": lambda m: self.count_food(m, "youngScrub", "woodland"),
                        # "Grass_Woodland": lambda m: self.count_food(m, "grass", "woodland"),
                        # # number in scrubland
                        # "Trees_scrub": lambda m: self.count_food(m, "trees", "thorny_scrubland"),
                        # "Saplings_scrub": lambda m: self.count_food(m, "saplings", "thorny_scrubland"),
                        # "Scrub_scrub": lambda m: self.count_food(m, "scrub", "thorny_scrubland"),
                        # "youngScrub_scrub": lambda m: self.count_food(m, "youngScrub", "thorny_scrubland"),
                        # "Grass_scrub": lambda m: self.count_food(m, "grass", "thorny_scrubland"),
                        # what's being eaten
    ...
    def remove_herbivores(self, herbivore, count):
        ...
        # my_choice = random.choice(list(to_remove), size = count)[1]
