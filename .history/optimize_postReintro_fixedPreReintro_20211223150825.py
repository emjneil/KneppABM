...
def objectiveFunction(x):
    ...
    ponies_gain_from_grass = x[6] 
    ponies_gain_from_Trees = x[7] 
    ponies_gain_from_Scrub = x[8] 
    ponies_gain_from_Saplings = x[9]  
    ponies_gain_from_YoungScrub = x[10]
    cows_reproduce =x[11] 
    cows_gain_from_grass = x[12] 
    cows_gain_from_Trees = x[13] 
    cows_gain_from_Scrub = x[14] 
    cows_gain_from_Saplings = x[15] 
    cows_gain_from_YoungScrub = x[16] 
    # pigs_reproduce =x[23] 
    ...
    if filtered_result < 25000:
        ...
        with pd.option_context('display.max_columns',None):
            ...
            print(just_nodes1[["Exmoor pony", "Fallow deer", "Longhorn cattle"]])
            print(just_nodes2[["Exmoor pony", "Fallow deer", "Longhorn cattle"]])
            print(just_nodes3[["Exmoor pony", "Fallow deer", "Longhorn cattle"]])
...
#  7.0
    ...
    optimization =  ga(function = objectiveFunction, dimension = 17, variable_type = 'real',variable_boundaries= bds, algorithm_parameters = algorithm_param, function_timeout=6000)
    optimization.run()
