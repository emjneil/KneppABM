...
def run_counterfactual():
    ...
    counterfactual['Run Status'] = "noReintro"
...
def forecasting():
    ...
    accepted_parameters = pd.read_csv('combined_accepted_parameters.csv') 
    accepted_parameters.drop(['Unnamed: 0'], axis=1, inplace=True)
    accepted_parameters = accepted_parameters.iloc[0:1]
    ...
    for index, row in accepted_parameters.iterrows():
        ...
        for i in range(params_to_check):
            ...
            model = KneppModel(initial_roe, roe_deer_reproduce, roe_deer_gain_from_saplings, roe_deer_gain_from_trees, roe_deer_gain_from_scrub, roe_deer_gain_from_young_scrub, roe_deer_gain_from_grass,
                    ...
                    fallowDeer_stocking_forecast, cattle_stocking_forecast, redDeer_stocking_forecast, tamworthPig_stocking_forecast, exmoor_stocking_forecast, introduced_species_stocking_forecast,
                    chance_scrub_saves_saplings,
                    max_time = 484, reintroduction = True, introduce_euroBison = False, introduce_elk = False, introduce_reindeer = False)

            # run each one 10 times
            model.run_model()
            final_results = model.datacollector.get_model_vars_dataframe()
            final_results['run_number'] = run_number
            final_results['param_run'] = i
            # append 
            my_param_outputs.append(final_results)
        # append to dataframe
        final_results = pd.concat(my_param_outputs)
        ...
        for index, row in my_time.iterrows():
            if (row["Roe deer"] <= 40) & (row["Roe deer"] >= 12) & (row["Grassland"] <= 80) & (row["Grassland"] >= 20) & (row["Woodland"] <= 17) & (row["Woodland"] >= 5.8) & (row["Thorny Scrub"] <= 51.8) & (row["Thorny Scrub"] >= 4.3):
                accepted_runs.append(row["param_run"])
        final_results['passed_filters'] = np.where(final_results['param_run'].isin(accepted_runs),final_results['passed_filters']+1,final_results['passed_filters']) 
        # April 2015
        ...
        # check each df based on the run number
        accepted_habs = []
        my_time = final_results.loc[final_results['Time'] == 49]
        for index, row in my_time.iterrows():
            if (row["Grassland"] <= 89.9) & (row["Grassland"] >= 26.8) &(row["Thorny Scrub"] <= 51.8) & (row["Thorny Scrub"] >= 4.3) &(row["Woodland"] <= 17) & (row["Woodland"] >=  5.8) &(row["Roe deer"] <= 40) & (row["Roe deer"] >= 12):
                accepted_habs.append(row["param_run"])
        accepted_df = final_results['param_run'].isin(accepted_habs)
        accepted_df_1 = final_results[accepted_df]

        # and last filters?
        accepted_last_year = []
        my_time = accepted_df_1.loc[accepted_df_1['Time'] == 183]
        for index, row in my_time.iterrows():
            if (row["Grassland"] <= 36.8) & (row["Grassland"] >= 16.8) &(row["Thorny Scrub"] <= 61.8) & (row["Thorny Scrub"] >= 41.8) &(row["Woodland"] <= 31.5) & (row["Woodland"] >=  11.5) &(row["Roe deer"] <= 80) & (row["Roe deer"] >= 20):
                accepted_last_year.append(row["param_run"])
        accepted_df_final = accepted_df_1['param_run'].isin(accepted_last_year)
        final_accepted_results = accepted_df_1[accepted_df_final]

        # take the top results
        best_result = final_accepted_results.nlargest(1, 'passed_filters')
        final_df = final_accepted_results['param_run'].isin(best_result['param_run'])
        best_result_df = final_accepted_results[final_df].drop("param_run", axis=1)

        # append the best one to the final df
        final_results_list.append(best_result_df)
